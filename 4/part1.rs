...
fn main() {
    ...
    for pair_of_elfs in elfs {
        ...
        if first_elf_end_with >= second_elf_end_with && first_elf_starts_from <= second_elf_starts_from{
            count += 1;
        } 
        else if second_elf_starts_from <= first_elf_starts_from && second_elf_end_with >= first_elf_end_with {
            count += 1;
        }
    }
    println!("{count}")
